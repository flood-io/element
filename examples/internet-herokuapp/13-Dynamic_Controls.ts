...
export default () => {
	...
	step('Test: 02 - Dynamic Controls', async (browser) => {
		...
		const pageTextVerify = By.visibleText('Dynamic Controls')
		await browser.wait(Until.elementIsVisible(pageTextVerify))
	})

	step('Test: 03 - Dynamic Controls - Checkbox Remove', async (browser) => {
		const Checkbox = await browser.findElement(By.css('#checkbox > input[type=checkbox]'))
		await Checkbox.click()
		const Remove = await browser.findElement(By.css('#checkbox-example > button'))
		await Remove.click()
	})

	step('Test: 04 - Dynamic Controls - Add', async (browser) => {
		const pageTextVerify = By.visibleText("It's gone!")
		await browser.wait(Until.elementIsVisible(pageTextVerify))
		const Remove = await browser.findElement(By.css('#checkbox-example > button'))
		await Remove.click()
	})
	...
	step('Test: 05 - Dynamic Controls - Is Back!', async (browser) => {
		const pageTextVerify = By.visibleText("It's back!")
		await browser.wait(Until.elementIsVisible(pageTextVerify))
	})
}
