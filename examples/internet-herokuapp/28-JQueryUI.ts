...
export default () => {
	...
	step('Test: 02 - JQueryUI', async (browser) => {
		...
		const pageTextVerify = By.visibleText('JQueryUI')
		await browser.wait(Until.elementIsVisible(pageTextVerify))
	})

	step('Test: 03 - Menu', async (browser) => {
		const Menu = await browser.findElement(By.css('#ui-id-3'))
		await Menu.focus()
		await Menu.click()
	})

	step('Test: 04 - SubMenu', async (browser) => {
		const SubMenu = await browser.findElement(By.css('#ui-id-4'))
		await SubMenu.focus()
		await SubMenu.click()
	})

	step('Test: 05 - PDF', async (browser) => {
		const PDF = await browser.findElement(By.css('#ui-id-5'))
		await PDF.focus()
		await PDF.click()
	})
}
