...
export default () => {
	...
	step('Test: 02 - Data Tables', async (browser) => {
		...
		const pageTextVerify = By.visibleText('Data Tables')
		await browser.wait(Until.elementIsVisible(pageTextVerify))
	})

	step('Test: 03 - Table 1', async (browser) => {
		const Table1 = await browser.findElement(
			By.css('#table1 > tbody > tr:nth-child(4) > td:nth-child(5)')
		)
		const Value1 = await Table1.text()
		console.log('Table 1 - Col 5 Row 4 Value: ' + Value1)

		const Table11 = await browser.findElement(
			By.css('#table1 > tbody > tr:nth-child(4) > td:nth-child(1)')
		)
	...
	step('Test: 04 - Table 2', async (browser) => {
		const Table2 = await browser.findElement(By.css('#table2 > tbody > tr:nth-child(2) > td.dues'))
		const Value2 = await Table2.text()
		console.log('Table 2 - Col 4 Row 2 Value: ' + Value2)

		const Table22 = await browser.findElement(
			By.css('#table2 > tbody > tr:nth-child(1) > td.email')
		)
