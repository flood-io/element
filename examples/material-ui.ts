import { TestSettings, step, By, Until, Browser } from '@flood/element'
...
export default () => {
	step('Step 1: Visit', async browser => {
		await browser.visit('https://material-ui.com/')
	})

	step.if(
		async browser => {
			const icon = await browser.findElement(
				By.css(
					'#__next > div > header > div > button:nth-child(9) > span.MuiIconButton-label > svg > path',
				),
			)
			// Run this step if default theme is dark
			return (await icon.getAttribute('d')) === iconSun
		},
