...
import { TestScriptErrorMapper, TestScriptError } from '@flood/element-report'
...
export function structuredErrorToDocumentedError(
	...
	switch (sErr.data._kind) {
		case 'assertion':
			return script.liftError?.(assertionError(sErr as StructuredError<AssertionErrorData>))
		case 'net':
			return script.liftError?.(netError(sErr as StructuredError<NetworkErrorData>))
		case 'action':
			return script.liftError?.(actionError(sErr as StructuredError<ActionErrorData>))
		case 'locator':
			return script.liftError?.(locatorError(sErr as StructuredError<LocatorErrorData>))
		case 'puppeteer':
			return script.liftError?.(puppeteerError(sErr as StructuredError<PuppeteerErrorData>))
	}
