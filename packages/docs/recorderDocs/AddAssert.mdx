...
An assertion is a validation step that determines whether the automated test scenario passed or failed, or, in other words, to check if the actual and expected result are equal to each other.

Element Recorder supports different common methods for assertion including:

* **Element assertion:** asserts if an element's attribute or property (eg: innerText) matches the expected value.

* **Page URL assertion:** asserts if a page URL's property (eg: pathname) matches the expected value.
...
<img src={useBaseUrl('docs_img/rec-assert-element-select.png')} alt="add element assertion - select element" width="600"/>

"Element assertion" screen will appear and you can edit the assertion details.

<img src={useBaseUrl('docs_img/rec-assert-element-details.png')} alt="add element assertion - edit details" width="250"/>
...
* **Target element**
  ...
  * If you'd like to change the element to assert, click `Change element` button on the top.
...
* **Assertion type**
  ...
  Allows you to select how the attribute or property above will be asserted against the expected value. A set of string and number comparison operators help you to form any assertion conditions. You can even use regular expression (`matches regex`) for intricate assertions.
...
* **Attribute to assert**
  ...
  <img src={useBaseUrl('docs_img/rec-assertion-pageurl-attribute.png')} alt="page url attribute" width="250"/>
...
* **Attribute to assert**
  ...
  <img src={useBaseUrl('docs_img/rec-assert-cookie-attribute.png')} alt="cookie attribute to assert" width="250"/>
