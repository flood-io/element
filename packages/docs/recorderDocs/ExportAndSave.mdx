...
To export a test script, on Recording screen, click `Export` button at the bottom.
...
1. On Recording screen, click `Save to Flood` button at the bottom.
...
2. The Flood upload configuration page will be opened in a new tab and will prompt you to log in to [Flood][flood] first if you're not logged in.
  ...
  <img src={useBaseUrl('docs_img/rec-flood-select-project.png')} alt="select a flood project" width="600"/>

  On this page, you can configure the script uploading as below.

   * Select a Flood project to upload the script to.
  
   * Select to "Create a new stream" or "Modify an existing stream". If you select to "Modify an existing stream", you'll need to select a stream to add this script to.
