...
Once launched, click `REC` button to start recording. The first action - `NAVIGATE` to the page - is created to get you started. Then just interact with the page and each of your actions will be recorded in the Recorder.

<img src={useBaseUrl('docs_img/rec-some-actions.png')} alt="record-some-actions" width="600"/>

To pause or resume recording, click `REC` button again.
...
To add a new step, click `Add` > `Step`, then continue interact with the page. The actions recorded from here will belong to this step.
...
During recording, if you'd like to make adjustment on a recorded action, hover on the action, then click `...` icon > `Edit action`. Edit action screen will appear.
...
* **Action type**
  ...
  * Allows you to switch between applicable action types (eg: change from `CLICK` to `DBCLICK`).
...
* **Action-specific fields**
  ...
  * **Target element**
    ...
    * You can also change the target element by manually enter the new selector to the field.

    * Supports switching between `CSS` and `XPATH` selector. By default, elements are selected using `CSS` selector. You can change this in the [Recorder settings][recorder-settings-xpath].
  ...
  * **Value fields** 
    * An action may have or may not have value fields. Each of the actions with values has a different set of value fields applicable for that particular action. For example, CLICK action doesn't have a value, TYPE action has 'Value to type', while RESIZE action has 'Viewport width' and 'Viewport height'. [See more][actions].
...
For `NAVIGATE` action, you can directly edit the URL on the Recording screen.

### Insert actions

To insert some actions before or after a recorded action:

1. Hover on the action, then click `...` icon > `Move cursor before` or `Move cursor after`. The red line (aka the cursor) will be moved to the corresponding position.

 <img src={useBaseUrl('docs_img/rec-action-context-menu.png')} alt="action context menu" width="250"/>

2. Continue interact with the page to record actions at the current cursor position.

### Duplicate an action

If you'd like to add another action that is quite similar to an action, you may want to duplicate the action and edit from there.

To duplicate an action, hover on the action, then click `...` icon > `Duplicate action`. A duplicate action will be created right after this action.

 <img src={useBaseUrl('docs_img/rec-action-context-menu.png')} alt="action context menu" width="250"/>

### Move an action

You can move a recorded action up or down, or to another step by drag-and-drop the action on Recording screen.

### Undo - Redo - Delete actions

* To undo or redo recorded actions by clicking the icons on the top-left of the Recorder.

* To delete an action, hover on the action, then click `x` icon.
...
Capture the screenshot of the page after an action is executed might be helpful in some situations, especially, after an [assertion][assert].
...
1. Hover on an action, then click `...` icon > `Add wait` to add a wait after this action.
