...
const debugSandboxScreenshot = debug('sandbox:screenshot')
...
export class Sandbox implements Driver {
	...
	constructor(
		...
		ensureDir(tracesDirectory).catch(err => {
			console.error(err)
		})

		ensureDir(networkDataDirectory).catch(err => {
			console.error(err)
		})
	}
	...
	public async visit(url: string, options: NavigationOptions = {}): Promise<void> {
		...
		await this.page.goto(url, { timeout, waitUntil: 'domcontentloaded', ...options })
		return
	}
	...
	public async interactionTiming(): Promise<any> {
		let polyfill = readFileSync(resolve(join(__dirname, '../extern/tti-polyfill-debug.js')), 'utf8')
